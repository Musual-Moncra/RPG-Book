...
function Keybind:OnLetGo(Player, Snapshot)
	...
	task.delay(2, function()
		for _, MobInstance in CollectionService:GetTagged("Mob") do
			...
			if MobTorso and (MobTorso.Position - Origin).Magnitude < 40 then
				task.spawn(function()
					RequestStunMob(MobInstance, 3)
					Knockback:Activate(MobTorso, 25, Origin, MobTorso.Position)
					Damage:DamageMobSkill(Player, MobList[MobInstance], {
						Damage = 150,
						Tool = Tool,
						SkillScaling = {
							["Intelligence"] = { Type = "Attributes", Additive = 2.0, Cap = 500 },
